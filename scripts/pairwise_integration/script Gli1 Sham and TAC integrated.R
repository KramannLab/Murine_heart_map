#script for analysis of Gli1 Sham and TAC integrated
library(Seurat)
library(cowplot)
library(ggplot2)
library(clustree)
...
Samples.combined = readRDS(file = "~/Documents/FP_scRNA/R stuff/analysis PVM_v8/RDS_basic_filtering_Sham&TAC_integrated/Gli1_Sham&TAC_integrated_seurat_object_after_bacis_filters.rds")
...
Samples.combined=subset(Samples.combined,idents = c("7"),invert=T)
...
Samples.combined=subset(Samples.combined,idents = c("6"),invert=T)
Samples.combined = recluster(Samples.combined,0.5)
...
Sham = subset(Samples.combined,idents = "FP13_Gli1_Sham")
saveRDS(Sham,file = paste0(sample,"pairwise/",sample,"_Sham_indiv_after_pairwise_integration.rds"))
TAC = subset(Samples.combined,idents = "FP12_Gli1_TAC")
