#script for analysis of Myh11 Sham and TAC integrated
library(Seurat)
library(cowplot)
library(ggplot2)
library(clustree)
...
Samples.combined = readRDS(file = "~/Documents/FP_scRNA/R stuff/analysis PVM_v8/RDS_basic_filtering_Sham&TAC_integrated/Myh11_Sham&TAC_integrated_seurat_object_after_bacis_filters.rds")
...
Samples.combined=subset(Samples.combined,idents = c("0","3","6","9","12"))
...
#----------subsetting to filter out tdtom low clusters-----------
Samples.combined=subset(Samples.combined,idents = c("5"),invert=T)
Samples.combined = recluster(Samples.combined,0.5)
...
Sham = subset(Samples.combined,idents = "FP56_Myh11_Sham")
saveRDS(Sham,file = paste0(sample,"pairwise/",sample,"_Sham_indiv_after_pairwise_integration.rds"))
TAC = subset(Samples.combined,idents = "FP57_Myh11_TAC")
