#script for analysis of PDGFRb Sham and TAC integrated
library(Seurat)
library(cowplot)
library(ggplot2)
library(clustree)
...
Samples.combined = readRDS(file = "~/Documents/FP_scRNA/R stuff/analysis PVM_v8/RDS_basic_filtering_Sham&TAC_integrated/PDGFRb_Sham&TAC_integrated_seurat_object_after_bacis_filters.rds")
...
Samples.combined=subset(Samples.combined,idents = c("4","11","12","14","15"),invert=T)
...
#---extra step for tdTom because of potential EC contamination cluster
VlnPlot(Samples.combined, features = c("tdTomato-Green"), pt.size = 0)
ggsave(filename = paste0(sample,"pairwise/processing_steps/",sample,"_after_frist_removal_tdTom_Vln.jpeg"), width=10 , height =10,limitsize = F)

#----------subsetting to filter out feature count low clusters-----------
Samples.combined=subset(Samples.combined,idents = c("7","8","13"),invert=T) # cluster 8 and 13 have weird tdtom expression pattern, cl 7 has very low feature count
Samples.combined = recluster(Samples.combined,0.5)
...
###---------------seperating Sham and Tac back to the individual samples-----------
Idents(Samples.combined)=Samples.combined@meta.data$orig.ident
DefaultAssay(Samples.combined)<-"integrated"
Sham = subset(Samples.combined,idents = "FP3_PDGFRb_Sham")
saveRDS(Sham,file = paste0(sample,"pairwise/",sample,"_Sham_indiv_after_pairwise_integration.rds"))
TAC = subset(Samples.combined,idents = "FP14_PDGFRb_TAC")
saveRDS(TAC,file = paste0(sample,"pairwise/",sample,"_TAC_indiv_after_pairwise_integration.rds"))

###---------------seperating Sham and Tac bac to the individual samples ONLY FIBRO CLUSTERS-----------
DefaultAssay(Samples.combined)<-"integrated"
Idents(Samples.combined)=Samples.combined@meta.data$seurat_clusters
Samples.combined.f = subset(Samples.combined,idents = c("6","7"),invert=T)
Idents(Samples.combined.f)=Samples.combined.f@meta.data$orig.ident
Sham = subset(Samples.combined.f,idents = "FP3_PDGFRb_Sham")
saveRDS(Sham,file = paste0(sample,"pairwise/",sample,"_Sham_indiv_after_pairwise_integration_ONLY_FIBRO.rds"))
TAC = subset(Samples.combined.f,idents = "FP14_PDGFRb_TAC")
saveRDS(TAC,file = paste0(sample,"pairwise/",sample,"_TAC_indiv_after_pairwise_integration_ONLY_FIBRO.rds"))

###---------------seperating Sham and Tac bac to the individual samples VSMCs and Pericytes-----------
DefaultAssay(Samples.combined)<-"integrated"
Idents(Samples.combined)=Samples.combined@meta.data$seurat_clusters
Samples.combined.f = subset(Samples.combined,idents = c("6","7"))
Idents(Samples.combined.f)=Samples.combined.f@meta.data$orig.ident
Sham = subset(Samples.combined.f,idents = "FP3_PDGFRb_Sham")
saveRDS(Sham,file = paste0(sample,"pairwise/",sample,"_Sham_indiv_after_pairwise_integration_ONLY_VSMC&Pericytes.rds"))
TAC = subset(Samples.combined.f,idents = "FP14_PDGFRb_TAC")
saveRDS(TAC,file = paste0(sample,"pairwise/",sample,"_TAC_indiv_after_pairwise_integration_ONLY_VSMC&Pericytes.rds"))